...
function shouldAutoInstallBrowser(): boolean {
  ...
  if (process.env.AUTODEPLOY_SKIP_BROWSER_INSTALL === '1') {
    return false;
  }
...
async function tryLaunchBrowser(chromium: ChromiumType): Promise<Browser> {
  ...
  if (envExecutable) {
    attempts.push({
      ...
      options: { executablePath: envExecutable, headless: true },
    });
  }
  ...
  if (bundled) {
    attempts.push({
      ...
      options: { executablePath: bundled, headless: true },
    });
  }

  for (const channel of ['chrome', 'msedge', 'chrome-beta'] as const) {
    attempts.push({ label: `channel:${channel}`, options: { channel, headless: true } });
  }
...
export function buildBrowserInstallHint(): string {
  ...
  return [
    ...
    '禁用自动安装: AUTODEPLOY_SKIP_BROWSER_INSTALL=1',
  ]
    .filter(Boolean)
    .join('\n');
}
